...
void setup_s21_is_not_equal(void) { is_not_equal = -10; }
...
START_TEST(positive_zero_and_negative_zero) {
  ...
  is_not_equal = s21_is_not_equal(first, second);

  ck_assert_int_eq(is_not_equal, S21_FALSE);
}
END_TEST
...
START_TEST(negative_zero_and_negative_zero) {
  s21_decimal first = S21_DECIMAL_NULL;
  s21_decimal second = S21_DECIMAL_NULL;
...
START_TEST(same_value_decimals) {
...
START_TEST(same_values_with_different_sign_is_not_equal) {
  ...
  ck_assert_int_eq(is_not_equal, S21_TRUE);
}
END_TEST

START_TEST(different_values) {
  s21_decimal first = {{0, 2, 4, 34}};
...
TCase *tcase_s21_is_not_equal(void) {
  ...
  tc = tcase_create("Tests for `s21_is_not_equal`");
  tcase_add_checked_fixture(tc, setup_s21_is_not_equal, NULL);

  tcase_add_test(tc, positive_zero_and_negative_zero);
  tcase_add_test(tc, positive_zero_and_positive_zero);
  tcase_add_test(tc, negative_zero_and_negative_zero);
  tcase_add_test(tc, same_value_decimals);
  tcase_add_test(tc, same_values_with_different_sign_is_not_equal);
  tcase_add_test(tc, different_values);
