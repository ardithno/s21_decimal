...
START_TEST(first_less_then_second) {
  ...
  ck_assert_int_eq(is_less, S21_TRUE);
}
END_TEST
...
TCase *tcase_s21_is_less(void) {
  ...
  tc = tcase_create("Tests for `s21_is_less`");
  tcase_add_checked_fixture(tc, setup_s21_is_less, NULL);
